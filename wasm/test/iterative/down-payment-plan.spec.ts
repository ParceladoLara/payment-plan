...
test("calculate down payment plan 2 installments", () => {
  ...
  const params: DownPaymentParams = {
    params: planParam,
  ...
  const planDueDate = new Date("2022-07-20T00:00:00.000Z"); //Node is a joke
  const firstPaymentDate = new Date(firstPlan.dueDate);
  firstPaymentDate.setUTCHours(0, 0, 0, 0);

  assert.deepEqual(firstPaymentDate, planDueDate);
  ...
  const planDueDate2 = new Date("2022-08-20T00:00:00.000Z"); //Node is a joke
  const firstPaymentDate2 = new Date(firstPlan2.dueDate);
  firstPaymentDate2.setUTCHours(0, 0, 0, 0);
  assert.deepEqual(firstPaymentDate2, planDueDate2);
});
