...
impl PaymentPlan for Iterative {
    fn calculate_payment_plan(
        ...
        if min_installments > params.installments || min_installments == 0 {
            return Err(PaymentPlanError::InvalidNumberOfInstallments);
        }
...
mod test {
    ...
    #[test]
    fn test_iterative_with_min_installments_set() {
        let disbursement_date = chrono::NaiveDate::from_ymd_opt(2024, 10, 23).unwrap();
        ...
        let params = Params {
            ...
            iof_percentage: 0.000082, // 0.0082%
            interest_rate: 0.035,
            min_installments: Some(2),
        };

        let iterative = Iterative;
        let mut resp = iterative.calculate_payment_plan(params).unwrap();
        assert_eq!(resp.len(), 47);

        let first_element = resp.first().unwrap();
        assert_eq!(first_element.installment, 2);
    ...
    #[test]
    fn test_min_installments_greater_than_installments() {
        let disbursement_date = chrono::NaiveDate::from_ymd_opt(2024, 10, 23).unwrap();
