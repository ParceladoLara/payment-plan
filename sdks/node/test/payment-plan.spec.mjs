...
test('calculate payment plan test 1 (with min installment)', () => {
  /**
   * @type {PaymentPlanParams}
   */
  const params = {
  ...
  assert.equal(result.length, 23); //The 1 installment result should be removed because of the minInstallments parameter
  const pop = result.pop();
  /**
   * @type {PaymentPlanResponse}
   */
