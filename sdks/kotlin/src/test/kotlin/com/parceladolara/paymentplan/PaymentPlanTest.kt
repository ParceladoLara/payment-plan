...
class PaymentPlanTest {
        ...
        /**
         * Helper assert function similar to Go tests. Rounds Double values to 10 decimal places for
         * cross-platform comparison. This helps avoid precision issues between Windows and Linux.
         */
        private fun assertEqualsWithRounding(
                expected: Double,
                actual: Double,
                message: String? = null
        ) {
                val expectedRounded =
                        BigDecimal(expected).setScale(10, RoundingMode.HALF_UP).toDouble()
                val actualRounded = BigDecimal(actual).setScale(10, RoundingMode.HALF_UP).toDouble()
                assertEquals(
                        expectedRounded,
                        actualRounded,
                        message ?: "Values should be equal when rounded to 10 decimal places"
                )
        }
        ...
        fun `test calculatePaymentPlan - single installment`() {
                ...
                // Verify basic functionality - amount should be higher than requested due to
                // interest
                val firstInstallment = result[0]
                assertTrue(
                        firstInstallment.installmentAmount > 7800.0,
                        "Installment amount should be higher than requested amount due to interest"
                )
                assertTrue(
                        firstInstallment.totalAmount > 7800.0,
                ...
                assertEqualsWithRounding(
                        7800.0,
                        firstInstallment.preDisbursementAmount,
                ...
                val daysDifference =
                        Math.abs(
                                actualDisbursement.epochSecond - requestedDisbursement.epochSecond
                        ) / 86400
                assertTrue(
                        daysDifference <= 7,
        ...
        @Test
        fun `test calculatePaymentPlan - with min installment`() {
                // Using adapted test data from Go SDK for single installment
                val params =
                        Params(
                                requestedAmount = 7800.0,
                ...
                assertEquals(
                        result.size,
                        3
                ) // Should return 3 installments due to minInstallments constraint
                ...
                assertTrue(
                        firstInstallment.installmentAmount < 7800.0,
                        "Installment amount should be lower than requested amount due to multiple installments"
                )
                assertTrue(
                        firstInstallment.totalAmount > 7800.0,
                        "Total amount should be higher than requested amount due to interest"
                ...
                // Verify basic structure
                assertEquals(2u, firstInstallment.installment)
                // Don't compare exact dates as they may be adjusted for business days
                assertNotNull(firstInstallment.disbursementDate)
        }
        ...
        fun `test calculateDownPaymentPlan`() {
                ...
                assertTrue(
                        firstResult.installmentAmount > 0,
                        "Installment amount should be positive"
                )
                assertTrue(firstResult.totalAmount > 0, "Total amount should be positive")

                // Verify basic structure
                assertTrue(
                        firstResult.installmentQuantity > 0u,
                        "Should have at least one installment"
                )
                assertNotNull(firstResult.firstPaymentDate)
                assertNotNull(firstResult.plans)
                assertTrue(firstResult.plans.isNotEmpty(), "Should have payment plan details")
        }
        ...
        fun `test disbursementDateRange`() {
                ...
                val baseDate =
                        ZonedDateTime.of(2025, 4, 3, 0, 0, 0, 0, ZoneId.of("UTC")).toInstant()
                val days = 5u
                ...
                assertTrue(endDate.isAfter(startDate), "End date should be after start date")
                ...
                assertTrue(
                        daysDifference >= days.toLong() - 2 && daysDifference <= days.toLong() + 5,
                        "Date range should span approximately $days business days (allowing for weekends)"
                )

                // Verify dates are adjusted for business days (should be in Brazil timezone with 7
                // AM)
                ...
                assertTrue(
                        startZoned.hour == 7 || startZoned.hour == 0,
                        "Start date should be adjusted for business hours"
                )
                assertTrue(
                        endZoned.hour == 7 || endZoned.hour == 0,
        ...
        fun `test nextDisbursementDate`() {
                ...
                assertTrue(
                        !resultDateBrazil.isBefore(baseDateBrazil),
        ...
        fun `test getNonBusinessDaysBetween`() {
                // Using same test data as Go SDK
                val startDate =
                        ZonedDateTime.of(2025, 4, 1, 0, 0, 0, 0, ZoneId.of("UTC")).toInstant()
                val endDate =
                        ZonedDateTime.of(2025, 4, 30, 0, 0, 0, 0, ZoneId.of("UTC")).toInstant()

                val nonBusinessDays = PaymentPlan.getNonBusinessDaysBetween(startDate, endDate)

                assertNotNull(nonBusinessDays)
                assertTrue(
                        nonBusinessDays.isNotEmpty(),
        ...
        fun `test calculatePaymentPlan - precise decimal comparison`() {
                ...
                val params =
                        Params(
                                requestedAmount = 1000.0,
                                firstPaymentDate =
                                        ZonedDateTime.of(2025, 6, 3, 10, 0, 0, 0, ZoneId.of("UTC"))
                                                .toInstant(),
                                disbursementDate =
                                        ZonedDateTime.of(2025, 5, 3, 10, 0, 0, 0, ZoneId.of("UTC"))
                                                .toInstant(),
                                installments = 3u,
                ...
                if (firstInstallment.accumulatedDaysIndex > 0) {
                        assertEqualsWithRounding(
                                firstInstallment.accumulatedDaysIndex,
                                firstInstallment.accumulatedDaysIndex,
                ...
                val roundedInstallmentAmount =
                        BigDecimal(firstInstallment.installmentAmount)
                                .setScale(2, RoundingMode.HALF_UP)
                                .toDouble()
                val roundedTotalAmount =
                        BigDecimal(firstInstallment.totalAmount)
                                .setScale(2, RoundingMode.HALF_UP)
                                .toDouble()
                val roundedDaysIndex =
                        BigDecimal(firstInstallment.daysIndex)
                                .setScale(10, RoundingMode.HALF_UP)
                                .toDouble()
                val roundedAccumulatedDaysIndex =
                        BigDecimal(firstInstallment.accumulatedDaysIndex)
                                .setScale(10, RoundingMode.HALF_UP)
                                .toDouble()
